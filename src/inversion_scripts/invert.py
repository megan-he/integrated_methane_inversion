...
def do_inversion(
    ...
    # Load global 2x2.5 spline fit observational error here (all observations)
    obs_error_full = np.load("/n/holylfs05/LABS/jacob_lab/Users/mhe/Global_2020_annual/so_est.npy")
    obs_error_all = obs_error_full.copy()
    ...
    for fi in files:
        ...
        m = obs_GC.shape[0] # number of observations in this orbit
        obs_error = obs_error_full[:m]
        # check to make sure obs_err isn't negative, set 1 as default value
        obs_error = [obs if obs > 0 else 1 for obs in obs_error]
        # Update obs_error_full to remove the errors already used
        obs_error_full = obs_error_full[m:]
